...
系统 SHALL 支持 DeepSeek、DashScope、OpenAI 三类 Provider。Provider 凭据（api_key、base_url、model）由每位用户独立配置并存储于数据库，不再依赖服务端环境变量。所有 Provider 使用统一的 OpenAI-compatible SDK 调用，仅切换 `base_url` 和 `api_key`。
